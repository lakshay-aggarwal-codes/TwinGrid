...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const hours = parseInt(pathParts[pathParts.length - 1]) || 24;
    ...
    for (let i = 0; i < hours; i++) {
      const variance = 0.05;
      const util_variation = utilisation + (Math.random() - 0.5) * variance;
      const temp_variation = inlet_temp + (Math.random() - 0.5) * 2;

      const result = simulate(
        Math.max(0.3, Math.min(1.0, util_variation)),
        Math.max(15, Math.min(30, temp_variation)),
        cooling_mode
      );

      timeSeries.push({
        timestamp: now - (hours - i - 1) * 3600000,
        hour: i,
        ...result
      });
    }
